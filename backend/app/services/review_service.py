...
async def get_product_reviews(product_id: str, limit: int = 30) -> Dict[str, Any]:
    ...
    reviews = (
        await db.reviews.find({"product_id": oid})
        .sort("created_at", -1)
        .limit(limit)
        .to_list(limit)
    )
    ...
    buyers = (
        await db.users.find(
            {"_id": {"$in": buyer_ids}},
            {"full_name": 1, "profile_image_key": 1, "profile_image_url": 1},
    ...
    for review in reviews:
        ...
        review["buyer_image_url"] = resolve_file_url(
            buyer.get("profile_image_key") or buyer.get("profile_image_url")
        )
