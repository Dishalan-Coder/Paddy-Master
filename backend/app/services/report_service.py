...
async def get_admin_analytics() -> Dict[str, Any]:
    ...
    return serialize_document({
        ...
        "pending_products": await db.products.count_documents({"status": "active", "views": 0}),
        "active_listings": await db.products.count_documents({"status": "active"}),
        "total_orders": await db.orders.count_documents({}),
        "payment_processing": await db.orders.count_documents({"payment_status": "processing"}),
    })


async def get_farmer_dashboard(farmer_id) -> Dict[str, Any]:
    db = get_database_or_raise()
    upcoming_harvests = await db.crops.find(
        {"farmer_id": farmer_id, "growth_stage": {"$ne": "harvested"}}
    ).sort("expected_harvest_date", 1).limit(3).to_list(3)
...
async def get_buyer_dashboard(buyer_id) -> Dict[str, Any]:
    ...
    return {
        ...
        "active_orders": await db.orders.count_documents({"buyer_id": buyer_id, "status": {"$in": active_statuses}}),
        "completed_orders": await db.orders.count_documents({"buyer_id": buyer_id, "status": "delivered"}),
