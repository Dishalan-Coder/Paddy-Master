...
async def create_crop(data: CropCreate, user=Depends(require_farmer)):
    try:
...
@router.get("/")
async def get_crops(user=Depends(require_farmer)):
    return await crop_service.get_crops(user["_id"])


@router.get("/{crop_id}")
async def get_crop(crop_id: str, user=Depends(require_farmer)):
...
@router.put("/{crop_id}")
async def update_crop(crop_id: str, data: CropUpdate, user=Depends(require_farmer)):
    try:
...
async def delete_crop(crop_id: str, user=Depends(require_farmer)):
