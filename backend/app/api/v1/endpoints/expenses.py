...
async def add_expense(data: ExpenseCreate, user=Depends(require_farmer_general_access)):
    try:
...
async def get_expenses(
    crop_id: str | None = Query(None), user=Depends(require_farmer_general_access)
):
    return await expense_service.get_expenses(user["_id"], crop_id)


@router.get("/profit-loss")
async def get_profit_loss(user=Depends(require_farmer_general_access)):
