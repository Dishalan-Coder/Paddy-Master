...
export default function Sidebar({ open, onClose }) {
  ...
  const { user } = useAuth();
  const links = user?.role === 'admin' ? adminLinks : user?.role === 'buyer' ? buyerLinks : farmerLinks;
  return <>
    ...
    <aside className={`fixed left-0 top-0 z-50 flex h-full w-72 flex-col bg-[#10291f] shadow-2xl transition-transform duration-200 ${open ? 'translate-x-0' : '-translate-x-full'} lg:static lg:z-auto lg:translate-x-0`}>
      ...
      <nav className="flex-1 space-y-1 overflow-y-auto px-3 py-3">{links.map((link) => <NavLink key={link.to} to={link.to} end={link.to === '/dashboard' || link.to === '/admin'} onClick={onClose} className={({ isActive }) => isActive ? 'sidebar-link-active' : 'sidebar-link'}><link.icon className="h-5 w-5 shrink-0" /><span>{t(link.label, { defaultValue: link.label.replace('_', ' ') })}</span></NavLink>)}</nav>
      <div className="border-t border-white/10 p-4"><div className="flex items-center gap-3 rounded-2xl bg-white/5 p-3"><div className="grid h-10 w-10 shrink-0 place-items-center overflow-hidden rounded-xl bg-emerald-600 text-sm font-black text-white">{user?.profile_image_url ? <img src={user.profile_image_url} alt="" className="h-full w-full object-cover" /> : user?.full_name?.charAt(0) || 'U'}</div><div className="min-w-0 flex-1"><p className="truncate text-sm font-black text-white">{user?.full_name}</p><p className="text-xs capitalize text-emerald-200/60">{user?.role}</p></div></div></div>
    </aside>
  </>;
}
