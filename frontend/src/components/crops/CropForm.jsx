...
import { fieldClass, hasErrors, isAfterDate, toDateInputValue, toPositiveNumber } from '../../utils/forms';
...
export default function CropForm({ initialData, onSubmit, loading }) {
  ...
  useEffect(() => {
    ...
    farmService.getAll()
      ...
      .catch(() => { if (mounted) setError('Could not load farms. Create a farm before adding crops.'); })
      .finally(() => { if (mounted) setFarmsLoading(false); });
  ...
  const validate = () => {
    ...
    if (!form.expected_harvest_date) next.expected_harvest_date = 'Expected harvest date is required.';
    ...
    if (isAfterDate(form.planting_date, form.expected_harvest_date)) {
      next.expected_harvest_date = 'Harvest date must be after the planting date.';
    }
    if (notes.length > 500) next.notes = 'Notes must be 500 characters or less.';
    ...
    <form onSubmit={submit} className="space-y-5" noValidate>
      ...
      {!farms.length && (
        <div className="rounded-xl border border-amber-200 bg-amber-50 p-4 text-sm font-semibold text-amber-800">
          Add a farm before creating crop records. <Link to="/farms/new" className="font-black underline">Create farm</Link>
        </div>
      )}
      <div>
        <label htmlFor="crop_farm" className="label">Farm</label>
        <select
          id="crop_farm"
          name="farm_id"
          value={form.farm_id}
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        <div>
          <label htmlFor="crop_variety" className="label">Variety</label>
          <select
            id="crop_variety"
            name="variety"
            value={form.variety}
        ...
        <div>
          <label htmlFor="crop_area" className="label">Area (acres)</label>
          <input
            id="crop_area"
            name="area_acres"
            type="number"
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        <div>
          <label htmlFor="crop_planting_date" className="label">Planting date</label>
          <input
            ...
            aria-describedby={errors.planting_date ? 'planting_date-error' : undefined}
          />
          {fieldError('planting_date')}
        </div>
        <div>
          <label htmlFor="crop_harvest_date" className="label">Expected harvest date</label>
          <input
            ...
            aria-describedby={errors.expected_harvest_date ? 'expected_harvest_date-error' : undefined}
          />
          {fieldError('expected_harvest_date')}
        </div>
      </div>
      <div>
        <label htmlFor="crop_growth_stage" className="label">Growth stage</label>
        <select id="crop_growth_stage" name="growth_stage" value={form.growth_stage} onChange={change} className="input-field">
          {GROWTH_STAGES.map((stage) => <option key={stage} value={stage}>{formatGrowthStage(stage)}</option>)}
        </select>
      </div>
      <div>
        <label htmlFor="crop_notes" className="label">Notes</label>
        <textarea
          id="crop_notes"
          name="notes"
          value={form.notes}
          ...
          aria-describedby={errors.notes ? 'notes-error crop_notes_count' : 'crop_notes_count'}
        />
        <div className="mt-1 flex items-center justify-between gap-2">
          {fieldError('notes') || <span />}
          <span id="crop_notes_count" className="text-xs font-semibold text-slate-400">{form.notes.length}/500</span>
        </div>
      </div>
      <Button type="submit" loading={loading} disabled={!farms.length}>Save crop</Button>
    </form>
  );
}
