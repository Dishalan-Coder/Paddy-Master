...
export default function ReviewModal({ order, onClose, onSuccess }) {
  ...
  const submit = async (event) => {
    ...
    setError('');
    try {
      await reviewService.create(order.product_id, {
        order_id: order._id,
        rating,
        ...
        <form onSubmit={submit} className="mt-5 space-y-5" noValidate>
          ...
          <div>
            ...
            <div
              ...
              aria-describedby={
                errors.rating ? 'review_rating-error' : undefined
              }
            >
              {[1, 2, 3, 4, 5].map((star) => (
                <button
          ...
          <div>
            ...
            <textarea
              ...
              aria-describedby={
                errors.comment
                  ? 'review_comment-error review_comment_count'
