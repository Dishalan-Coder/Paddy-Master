...
export default function CropsPage() {
  ...
  const remove = async (crop) => {
    const cropName =
      typeof crop === 'string' ? 'this crop' : `${crop.variety} crop`;
    ...
    try {
      ...
      setActionError(
        getApiErrorMessage(requestError, 'Could not delete the crop.'),
      );
    }
  };
      ...
      <div className="flex flex-wrap items-end justify-between gap-4">
        <div>
          ...
          <p className="page-copy">
            Monitor growth stages, acreage, planting dates, and expected harvest
            windows.
          </p>
        </div>
        <Link
      ...
      {!loading && !crops?.length ? (
        <div className="rounded-[1.5rem] border border-dashed border-slate-300 bg-white py-16 text-center">
          ...
          <p className="mt-1 text-sm text-slate-400">
            Crop records unlock expense tracking and recommendations.
          </p>
