...
const getServerFieldErrors = (requestError) => {
  ...
  return detail.reduce((fieldErrors, item) => {
    const field = [...(item?.loc || [])]
      .reverse()
      .find((part) => PROFILE_FIELDS.includes(part));
    if (field && !fieldErrors[field]) {
      fieldErrors[field] = normalizeServerMessage(
...
export const getChangedProfilePayload = (
  currentProfile = {},
  nextValues = {},
) => {
  const currentValues = buildProfileUpdatePayload(
    getProfileFormValues(currentProfile),
  );
            ...
            <div className="px-6 pb-6 text-center">
              <div className="relative mx-auto -mt-12 h-24 w-24">
                {user?.profile_image_url ? (
                  ...
                  <div className="grid h-full w-full place-items-center rounded-[1.5rem] border-4 border-white bg-emerald-100 text-emerald-800 shadow-lg">
                    <UserRound className="h-9 w-9" />
                  </div>
                )}
                <label
                  className={clsx(
                    'absolute -bottom-2 -right-2 grid h-9 w-9 place-items-center rounded-xl bg-slate-950 text-white shadow-lg',
                    photoLoading
              ...
              <div className="mt-4 flex flex-wrap justify-center gap-2">
                <span
                  className={`inline-flex items-center gap-1 rounded-full px-3 py-1 text-xs font-black ${user?.is_verified ? 'bg-emerald-100 text-emerald-700' : 'bg-amber-100 text-amber-700'}`}
                >
                  {user?.is_verified ? (
